...
- Separate frontend apps built with Next.js + TypeScript:
  ...
  - Dashboard exposes a local settings proxy route (`GET/POST /api/settings/ai-chatbot`) that forwards to backend settings API using `NEXT_PUBLIC_API_BASE_URL + /settings/ai-chatbot` (with alias fallback) and normalizes JSON-only responses.
